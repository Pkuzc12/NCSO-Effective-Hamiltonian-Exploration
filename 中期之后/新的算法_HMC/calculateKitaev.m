function [E] = calculateKitaev(nei,A,x,y,leftflag,K,G,Gs,Kp,Gp,Gps)
...
if leftflag
    ...
    x_nei=nei(x,y).left.neighbor(1,1);
    y_nei=nei(x,y).left.neighbor(1,2);
    ...
    orient1=A(x,y).left.orientation;
    x_nei=nei(x,y).left.neighbor(2,1);
    y_nei=nei(x,y).left.neighbor(2,2);
    orient2=A(x_nei,y_nei).right.orientation;
    ...
    x_nei=nei(x,y).right.neighbor(1,1);
    y_nei=nei(x,y).right.neighbor(1,2);
    ...
    orient1=A(x,y).right.orientation;
    x_nei=nei(x,y).right.neighbor(2,1);
    y_nei=nei(x,y).right.neighbor(2,2);
    orient2=A(x_nei,y_nei).left.orientation;
    orient1=order(4:6,:)*U*orient1;
    orient2=order(4:6,:)*U*orient2;
    E=E+dot(orient1,Jp*orient2);
